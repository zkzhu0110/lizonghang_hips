...
class Customer {
 public:
  ...
  inline void Accept(const Message& recved) {
    if ((recved.meta.head == 0 || recved.meta.head == 3) && recved.meta.sender % 2 == 1
        && recved.meta.request == true && recved.meta.push == false && recved.meta.simple_app == false) {
 ...
 private:
  ...
  ThreadsafeQueue<Message> recv_queue_, recv_pull_queue_;
