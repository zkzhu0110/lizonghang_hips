...
struct Control {
  ...
  std::string DebugString() const {
    ...
    std::vector<std::string> cmds = {
      "EMPTY", "TERMINATE", "ADD_NODE", "ADD_GLOBAL_NODE", "BARRIER", "BARRIER_GLOBAL", "ACK", "HEARTBEAT"};
    std::stringstream ss;
    ss << "cmd=" << cmds[cmd];
    if (node.size()) {
      ss << ", node={";
  ...
  /** \brief all commands */
  enum Command { EMPTY, TERMINATE, ADD_NODE, ADD_GLOBAL_NODE, BARRIER, BARRIER_GLOBAL, ACK, HEARTBEAT };
  /** \brief the command */
  Command cmd;
  /** \brief node infos */
