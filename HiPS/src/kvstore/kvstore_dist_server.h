...
class KVStoreDistServer {
 ...
 private:
  ...
  void DataPullFromGlobalServersDefault(const DataHandleType type,
                                        ...
                                        ps::KVServer<char>* server) {
    CHECK(!ps::IsGlobalServer()) << "invalid pull operation on global servers";
    const bool is_compressed = gradient_compression_->get_type() != CompressionType::kNone;
    ...
    const int dtype = stored.dtype();
    const int num_bytes = mshadow::mshadow_sizeof(dtype);
    const int cmd = GetCommandType(is_compressed ? RequestType::kCompressedPushPull :
                                                   RequestType::kDefaultPushPull, dtype);
    ...
    if (stored.is_none()) {
      ...
      if (ps::IsGlobalServer()) {
        ...
        if (updates.request.size() == (size_t) ps::NumWorkers()) {
          ...
          const bool is_compressed = gradient_compression_->get_type() != CompressionType::kNone;
          int ts = is_compressed ? DataPushToGlobalServersCompressed(type, key, server) :
                                   DataPushToGlobalServersDefault(type, key, server);
          mu_.lock();
          ts_key_map_[ts] = key;
          mu_.unlock();
        } else {
