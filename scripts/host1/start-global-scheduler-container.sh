...
# container
IMAGE_NAME=hips
CONTAINER_NAME=hips-global-scheduler
HOST_NAME=global-scheduler
CONTAINER_PORT=9090
SSH_PORT=9000

# app
DMLC_ROLE_GLOBAL=global_scheduler 
DMLC_PS_GLOBAL_ROOT_URI=172.17.34.3
DMLC_PS_GLOBAL_ROOT_PORT=$CONTAINER_PORT 
DMLC_NUM_GLOBAL_SERVER=1 
DMLC_NUM_GLOBAL_WORKER=2
PS_VERBOSE=1
DMLC_INTERFACE=eth0

sudo nvidia-docker run -dit \
                --name $CONTAINER_NAME \
                -h $HOST_NAME\
                --cpus 8 \
                -m 12G \
                --memory-swap -1 \
                --shm-size 2G \
                -e DMLC_ROLE_GLOBAL=$DMLC_ROLE_GLOBAL \
                -e DMLC_PS_GLOBAL_ROOT_URI=$DMLC_PS_GLOBAL_ROOT_URI \
                -e DMLC_PS_GLOBAL_ROOT_PORT=$DMLC_PS_GLOBAL_ROOT_PORT \
                -e DMLC_NUM_GLOBAL_SERVER=$DMLC_NUM_GLOBAL_SERVER \
                -e DMLC_NUM_GLOBAL_WORKER=$DMLC_NUM_GLOBAL_WORKER \
...
# reallocate cpu and memmory.
# NOTE: this will disable nvidia-driver
sudo docker update -c 4 -m 256M $CONTAINER_NAME